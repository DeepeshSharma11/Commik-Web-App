...
@router.get("/")
async def get_milk_logs(log_date: Optional[date] = None, buffalo_id: Optional[str] = None, user=Depends(get_current_user)):
...
async def add_milk_log(data: MilkLogCreate, user=Depends(get_current_user)):
    ...
    # 1. Verify buffalo ownership before logging milk
    buffalo_check = supabase.table("buffaloes").select("owner_id").eq("id", data.buffalo_id).execute()
    if not buffalo_check.data or (buffalo_check.data[0]["owner_id"] != user["id"] and user.get("role") != "malik"):
    ...
    try:
        ...
        res = supabase.table("milk_logs").insert(insert_data).execute()
        return res.data[0]
