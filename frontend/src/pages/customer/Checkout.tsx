...
const Checkout = () => {
  ...
  if (step === 'success') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] animate-in zoom-in duration-500">
        ...
        <button onClick={() => navigate('/user/orders')} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition">
          View My Orders
        </button>
      </div>
    );
      ...
      <div className="min-h-[600px] flex items-center justify-center animate-in fade-in duration-300">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl max-w-md w-full border border-slate-100 dark:border-slate-700 space-y-6">
            ...
            <div className="space-y-4">
              {paymentSettings.qr_code_url && (
                <div className="flex justify-center">
                  <img src={paymentSettings.qr_code_url} alt="UPI QR Code" loading="lazy" className="w-48 h-48 border-4 border-blue-500 rounded-2xl object-contain bg-white p-2 shadow-lg" onError={e => (e.currentTarget.style.display = 'none')} />
                </div>
              )}
              <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4 border dark:border-slate-700">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(paymentSettings.upi_id); toast.success('UPI ID copied!'); }} className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg hover:bg-blue-200 transition text-blue-600"><Copy size={16} /></button>
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4 border dark:border-slate-700">
          ...
          <div className="pt-6 border-t dark:border-slate-700">
            ...
            <input type="text" placeholder="e.g. 31234567890" value={utr} onChange={e => setUtr(e.target.value)} className="w-full p-4 border dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-blue-500 outline-none transition font-mono" />
  ...
  // default 'address' step
  return (
    <div className="max-w-xl mx-auto space-y-6 animate-in fade-in duration-300">
      <h2 className="text-2xl font-black mb-6">Delivery Details</h2>
      <form onSubmit={handlePlaceOrder} className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 space-y-6">
        <div>
          <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-2"><MapPin size={16} className="text-blue-500" /> Delivery Address</label>
          <textarea required placeholder="Enter your full address (House No, Street, Landmark)..." value={address} onChange={e => setAddress(e.target.value)} rows={3} className="w-full p-4 border dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 outline-none focus:ring-2 focus:ring-blue-500 transition resize-none"></textarea>
        </div>
        <div>
          <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-2"><Clock size={16} className="text-blue-500" /> Preferred Delivery Time</label>
          <select value={timeSlot} onChange={e => setTimeSlot(e.target.value)} className="w-full p-4 border dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 outline-none focus:ring-2 focus:ring-blue-500 transition cursor-pointer appearance-none">
        ...
        <div className="pt-6 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
          ...
          <button type="submit" disabled={placing || !products.length} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold rounded-xl shadow-lg transition flex items-center gap-2">
